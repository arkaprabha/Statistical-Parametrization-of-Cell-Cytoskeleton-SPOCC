...
function [pmodel] = process( input_file, output_file, twosteps, verbose, pmodel, actualiter, numiter, tr, clean ),

addpath('../deepimg');
addpath('../gm');

if (nargin < 9), clean=0; end;

if (actualiter <= 1),
    [pmodel.img]=(t_deep_tifread(input_file, 1, 0.25)); pmodel.img=double(pmodel.img); pmodel.img=pmodel.img/max(pmodel.img(:))*255;
    pmodel.bimg=imresize(pmodel.img,1.0);
    ...
    if (twosteps),
        abimg=uint8(pmodel.bimg)*0.5+imadjust(uint8(imresize(pmodel.img,1.0)))*0.5;

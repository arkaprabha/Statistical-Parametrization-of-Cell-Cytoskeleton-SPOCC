...
mask=false(N,M);
mask(40:(end-40+1),40:(end-40+1))=true;
	...
	%for i=1:J
	  ...
	  % Cycle over dictionaries.
	   ...
	   % Solve for Parta the marginal penalized minimization problem (Hard thesholding, l_1 -> Soft).
	     ...
	     if (nb == comptv) & gamma~=0, 
             part(:,:,nb)  = TVCorrection(part(:,:,nb),gamma);
         else
           if (gamma~=0),
             % Rien
           end;
...
%            ti=part(:,:,nb);
%            ti=ti-min(ti(:)); ti=ti/max(ti(:));
%            imwrite(ti,['d:\Work\PhD\Projects\0-ESA Project\fiberAnalysisHough\SourceSeparation\tmp\' num2str(iter) '-sol.bmp']);
